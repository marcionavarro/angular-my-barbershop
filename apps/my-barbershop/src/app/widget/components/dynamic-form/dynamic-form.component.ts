...
export class DynamicFormComponent implements OnInit {
  ...
  /* private optionsCache = new WeakMap<any, Observable<any[]>>();

  protected getOptions(field: any): Observable<any[]> {
    const v = field?.select?.options$;
    if (!v) return of([]);
    if (isObservable(v)) return v as Observable<any[]>;
    // v é um array -> cachear o Observable criado
    if (this.optionsCache.has(field)) {
      return this.optionsCache.get(field)!;
    }
    const obs = of(v);
    this.optionsCache.set(field, obs);
    return obs;
  } */
    ...
    this.config?.forEach(control => {
      if (control.type.field !== eDynamicField.DIVIDER) {
        this.form.addControl(
          ...
          new FormControl(
            {
              value: control.initialValue !== undefined ? control.initialValue : null,
